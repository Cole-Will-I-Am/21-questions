...
// Cache portraits so re-runs (e.g. while tuning the judge) don't redo the 5 sessions. FRESH=1 forces.
const CACHE = "./cache";
fs.mkdirSync(CACHE, { recursive: true });
async function portraitFor(persona) {
  const f = `${CACHE}/${persona.id}.json`;
  if (process.env.FRESH !== "1" && fs.existsSync(f)) return JSON.parse(fs.readFileSync(f, "utf8"));
  const { portrait } = await runSession(persona);
  fs.writeFileSync(f, JSON.stringify(portrait, null, 2));
  return portrait;
}
...
async function judgeMatch(portrait, lineup) {
  ...
  return chatJSON({ model: JUDGE_MODEL, system: "You match a portrait to the one person it best describes. Decide on the evidence in the portrait; pick the single best match.", messages: [{ role: "user", content: user }], num_predict: 600, temperature: 0.2 });
}
async function selfRate(persona, portrait) {
  ...
  return chatJSON({ model: RATER_MODEL, system: "You rate how well a portrait matches you, the specific person described. 5 = uncannily specific and true; 1 = generic or wrong.", messages: [{ role: "user", content: user }], num_predict: 450, temperature: 0.3 });
}
...
const sessions = await Promise.all(PERSONAS.map(async (persona) => {
  try {
    ...
    console.log(`  · portrait ready: ${persona.id} → "${portrait.archetype}"`);
